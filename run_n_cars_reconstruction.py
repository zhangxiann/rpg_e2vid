...
if __name__ == "__main__":
    ...
    for train_test in environment.n_cars_train_test:
        for class_type in os.listdir(input_dir, train_test):
            index =0
            for file_name in os.listdir(os.path.join(input_dir, train_test, class_type)):
                # 每个数字生成 10 张就停止
                index =index+1
                if index >= 10:
                    break
                path_to_events = os.path.join(input_dir, train_test, class_type, file_name)
                args.dataset_name = file_name.split('.')[0]
                args.output_folder  = os.path.join('./data/output/n_cars', train_test, class_type)
